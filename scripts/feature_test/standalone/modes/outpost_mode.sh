...
Examples:
  ...
  ./scripts/feature_test/standalone/modes/outpost_mode.sh --offline --enable-fire true
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --mode)
      MODE_ARG="$2"
      st_warn "--mode is ignored by standalone outpost mode."
      shift 2
      ;;
    --wait)
      WAIT_SEC="$2"
...
if (( OFFLINE_MODE == 1 )); then
  st_warn "--offline is not supported by detector/outpost_test.launch.py; use config overrides for video/virtual IO if needed."
fi
...
LAUNCH_CMD=(
  ...
  "launch"
  "detector"
  "outpost_test.launch.py"
  "${LAUNCH_ARGS[@]}"
)

LAUNCH_LOG="${LOG_DIR}/outpost_mode_$(date +%Y%m%d_%H%M%S).log"
...
python3 "${ROOT_DIR}/scripts/feature_test/standalone/tools/target_to_control_bridge.py" \
  --target-topic "/ly/outpost/target" \
...
st_info "Outpost standalone mode running. Press Ctrl+C to stop."
set +e
