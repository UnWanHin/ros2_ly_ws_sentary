...
- `behavior_tree` 自己的状态机/任务开关配置放在 `src/behavior_tree/config/`，例如 `AreaManager.yaml`、`Task.yaml`；Default Base 巡邏點由程式固定，不再有 `Base.yaml` overlay。
...
| `ros2 launch gimbal_driver debug_node.launch.py` | 单节点 driver debug：内建 bridge 以 100 Hz 将 `/ly/navi/vel` 发往 `/ly/control/vel`、将 `/ly/navi/should_rotate` 发往 `/ly/control/firecode`，driver 只消费 formal control topics，不启动 BT/视觉/FaceMode |
