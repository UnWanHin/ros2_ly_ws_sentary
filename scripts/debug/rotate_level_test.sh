...
MODE="both"
INTERVAL_SEC=5
TX_TOPIC="/ly/control/firecode"
RX_TOPIC="/ly/gimbal/firecode"
ECHO_TOPIC="/ly/control/firecode"
LEVELS=(1 2 3)
LAUNCH_GIMBAL=1
WAIT_SEC=3
USE_VIRTUAL_DEVICE="false"
OUTPUT_MODE="screen"
CONFIG_FILE="${ROOT_DIR}/scripts/config/auto_aim_config_competition.yaml"
GIMBAL_PID=""
...
Modes:
  tx      Publish rotate levels in a loop: 1 -> 2 -> 3 -> ...
  rx      Echo rotate feedback topic (${RX_TOPIC})
...
launch_gimbal_driver() {
  ...
  echo "[ROTATE-TEST][INFO] Launching gimbal_driver with config=${CONFIG_FILE} use_virtual_device=${USE_VIRTUAL_DEVICE}" >&2
