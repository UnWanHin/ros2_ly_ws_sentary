...
# Pure static official-map -> navigation-map pointer conversion.
...
if use_static:
    ...
    if model not in {"matrix", "MATRIX", "matrix_4x4"}:
        print(
            f"[ERROR] OfficialToNavi.sh currently supports raw_goal_calibration_model=matrix only, got {model}",
            file=sys.stderr,
        )
        raise SystemExit(1)
