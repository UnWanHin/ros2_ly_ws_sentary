...
6. 请求优先级：普通评分为 `scored`；任务锁定的前哨强攻 `4` 和 ProtectHero 强防 `5`
   为 `task`；导航要求的硬移动为 `safety`；Recovery 强移 `6` 为 `recovery`。
   新请求只有在优先级更高时才会替换旧 pending，因此普通 Attack/Move 不会覆盖强制任务，
   低优先级 pending 也不会消耗一次尚未被接受的增强请求。

7. 三种增强姿态均通过同一 `PostureManager::Tick()` 和 `/ly/control/posture` 出口：
   - `4`：选中 ArmorType 7 且前哨有效扣血、强攻额度新鲜大于 0；回读 pending/active 后才算接受。
   - `5`：ProtectHero 到点后受击窗口达到阈值且强防额度可用；不会被通用受击分支降级。
   - `6`：Recovery 行进、正血低于配置阈值且强移额度可用；HP 为 0、资料过期或失败时回退 `3`。
