...
- 当 `Chase.ToNavi=true` 时，BT 不再执行本地追击速度闭环。追击优先使用 `/ly/aim/armor_targets`
  中当前选中目标的 point，按其 `header.frame_id`（默认 `gimbal_world`）发布 `/ly/navi/target_rel`，
  由 `navi_tf_bridge` 转成 `/goal_pose`。同时 bridge 会把 `/ly/aim/armor_targets` 中每个有效 target
