...
2. `src/gimbal_driver/config/debug_mode.yaml` 是唯一單節點 driver debug profile，完整擁有新版
...
4. 新增 `gimbal_driver/debug_node.launch.py` 作為只啟動 driver 的調試入口。它按順序載入
   正式基線與 `debug_mode.yaml`，可透過 `debug_config_file:=...` 指向其他未來的單節點
