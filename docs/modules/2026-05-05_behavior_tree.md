...
`Area.hpp` 的 `PreRoadland`、`ReadyRoadland` 是正式同級 `MainAreaKind`。`ReadyRoadland`
...
| `/ly/navi/reached` | `naviReach` | 外部到達證據；只由 `GoalReachState` 在 goal-scoped freshness 驗證後消費，不能直接推進任務 |
| `/ly/navi/reachable` | `naviReachable` | 外部可達性證據；只由 `GoalReachState` 統一產生不可達結論 |
| `/ly/navi/should_rotate` | `naviIsRotate` | 外部导航区域兼容旋转控制；true 恢复正常巡逻，false 关闭小陀螺并请求 FollowMode；`Navi.SetPostureToMoveWhenFalse=true` 时还会在消息新鲜期间请求 Move 姿态 |
...
- 自身哨兵坐標由 `AreaManager.SentryPositionFusion` 統一輸出到 `friendRobots[Sentry].position_`。`Mode=priority` 時按 `Priority` 選最新鮮的最高優先級源，默認順序是 `/ly/friend/uwb_pos`、`/ly/navi/position`、`/ly/position/data`；`Mode=weighted` 時對新鮮源按 `Weight` 加權平均。
...
- `StartGate.AllowGimbalPatrolBeforeStart`
  - YAML 開關在根目錄 `config/common.yaml` 的 `start_gate.allow_gimbal_patrol_before_start`；`true` 時 gated 啟動等待 `/ly/game/is_start=true` 期間只壓零底盤速度/小陀螺，雲台仍按 `PatrolScan.Mode` 掃描；start-gate pitch offset 在 `src/behavior_tree/config/Patrol.yaml` 的 `PatrolScan.TaskOverrides.StartGatePitchOffsetDeg`
- `DamageOpenGate`
...
- `StartGate.GimbalStrategy`
  - `patrol` 沿用 `PatrolScan.Mode`；正式 `Task.yaml` 目前選用 `patrol`，因此開局門控期間不會請求敵方前哨固定朝向。`face_mode_outpost` 才會持續向 `/ly/face_mode/target_raw` 請求敵方前哨固定朝向。缺省或非法值仍安全回退 `patrol`。
  ...
  - `FaceMode.Enable=false` 時，開賽後 Outpost/Buff 不發布固定點 FaceMode 請求，且 `/ly/face_mode/angles` 不會接管雲台；外部 `/ly/aim/result` 不受影響。
...
- `AimTargetPriority`：旧回退选靶优先级（按 `ArmorType` 整數 ID 排序）。当没有新鲜
  ...
  都可选，BT 选择最近目标。
- `src/behavior_tree/config/Aim.yaml`：临时 Aim 覆盖层，默认关闭。设置
  `Aim.Override.Enable=true` 后，`TargetPriority` 和 `TargetIgnore` 只替换明确提供的列表；
  未提供的列表继续使用竞赛 JSON。传入顺序是竞赛 JSON -> 其他模块 YAML -> `Aim.yaml` ->
  既有合法性检查、去重和默认回退，因此不会让默认空列表覆盖正式 profile。
- `Chase`：底盤追擊配置
  - `Enable`：總開關
...
`Area.MyArea.Highland.Task.MyHighland` 是区域任务框架的第一条任务：上游选中我方 Highland 大区点时，任务会按 `Highland(FollowMode+FaceMode)` -> `Highland` 驻留 15 秒并恢复巡逻/小陀螺 -> `BuffShoot` 到达后驻守 15 秒 -> `HoleRoad(FollowMode+FaceMode)` 离开。到达统一由 `GoalReachState` 判定，raw 导航状态与自身位置只作为其输入证据。

`Area.MyArea.Base.Task.MyBase` 管我方 Base 的无特别事件巡游：上游选中我方 Base 大区点时，只会在程式固定的 `CastleLeft1`、`CastleLeft2`、`CastleRight2`、`CastleRight1` 中按自身距离、当前点与访问新鲜度评分；到点后保持 15 秒，一轮最多四点。`BuffOutpost`、`HoleRoad`、`OutpostGuard` 不再是 Default Base 候选，前者只由 Buff/Outpost tactical 使用。到达统一消耗 `GoalReachState`。

`Area.MyArea.PreRoadland.Task.MyPreRoadland` 管我方道路前段：只前往 ID 25 `PreRoadland`，
到點後按 `GoalHoldSec=15` 秒結束；它取代舊 `Special.MiniRoadland`，並由 Default policy 的
獨立 score/retry/cooldown 選擇。`Area.MyArea.ReadyRoadland.Task.MyReadyRoadland` 管我方道路後段：
上游選中 ReadyRoadland 時，先去 `CentralToBase`；到點、不可達或超時後進入強綁定穿越段，
打開 `FollowMode`，並只在 `MyReadyRoadland.UseFaceMode=true` 時請求 FaceMode；直到到達
`BaseToCentral`、不可達或超時才恢復巡邏/開火/小陀螺。穿越段不會被 Buff/Outpost 等高優先級
邏輯直接取消；guard phase 保持 15 秒，approach/cross/return 仍是純行進 phase。
