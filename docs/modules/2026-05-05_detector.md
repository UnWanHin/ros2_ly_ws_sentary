...
> 当前定位：`Behavion` 正式 `sentry_all` 主链不启动本仓 `detector`。正式比赛的相机和识别结果由外部 `sentry.aim` 提供，并通过 `/ly/aim/*` 接入 `behavior_tree`。本文档保留给 legacy internal auto-aim、模块单测、标定和调试链路。
...
以下入口用于 legacy internal auto-aim 或模块调试，不是当前正式比赛主入口：
...
| 變量 | 類型 | 說明 |
|------|------|------|
| `myTeamRed` | `atomic_bool` | 我方是否紅隊（由 `/ly/friend/is_team_red` 更新） |
...
| Topic | 回調函數 | 作用 |
|-------|----------|------|
| `/ly/friend/is_team_red` | `my_team_callback` | 更新 `myTeamRed`，控制過濾器保留哪種顏色的裝甲板 |
