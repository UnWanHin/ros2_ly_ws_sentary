...
- `Default`：只做新的底層選點入口：按 `AreaManager.DefaultPolicy` 在已啟用的大區域中根據血量/彈量、距離、目前區域、上次任務結果、冷卻和重試評分，啟動 AreaManager 任務；沒有可用區域時才 fallback 到 `DecisionAutonomy.NaviGoal(HitHero)`，不再落回舊 HitHero fallback 點表。
