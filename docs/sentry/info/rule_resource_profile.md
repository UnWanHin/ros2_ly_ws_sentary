...
本文記錄 `docs/rules` 裡和哨兵血量、彈量、復活無敵相關的賽規數值，並對照目前 `gimbal_driver -> behavior_tree` 的數據流。
...
- `docs/rules/RoboMaster 2026 机甲大师超级对抗赛比赛规则手册V1.4.2（20260430）.pdf`
...
- `docs/rules/RoboMaster 2026 机甲大师高校系列赛通信协议 V2.0.0（20260626）.pdf`
- `docs/rules/2025/RoboMaster 2025 机甲大师超级对抗赛比赛规则手册V2.1.0（20250802）.pdf`
...
來源：超級對抗賽 V2.0.1，PDF 第 97 頁（規則頁 96）。

### 虛擬護盾：規則與現車可見性

基地的虛擬護盾先承受傷害；其扣除**不算基地血量損失**，但會計入對方全隊造成的
總傷害。V2.0.1 的來源有兩種：

| 來源 | 數值 | 規則效果 |
|---|---:|---|
| 開局 | `150` | 開局即存在，先於基地血量扣除。 |
| 四級裝配 | 基地加 `2000` 血時的溢出量 | 溢出部分轉為等值虛擬護盾，後續同樣優先扣除。 |

基地與前哨站的關係也需要注意：己方前哨站存活時，己方基地處於無敵狀態；前哨首次被毀
後，基地才可能受到攻擊。因此開局護盾不是「前哨存活時持續掉血」的可觀測信號。

現車鏈路**沒有**虛擬護盾 ROS topic 或上行 TypeID 欄位。`TypeID=2`
`HealthMyselfData.BaseMyself` 只原樣發布為 `/ly/friend/base_hp`，`TypeID=3` 的
`BaseEnemy` 原樣發布為 `/ly/enemy/base_hp`；它們是 `uint16` 的基地當前血量，不含
虛擬護盾。官方 `0x0003 game_robot_HP_t` 也只提供基地血量欄位。

所以 `base_hp` 維持 `5000` 時可能是「未被攻擊」、也可能是「正在扣 150 開局護盾」、
也可能是「正在扣四級裝配的溢出護盾」。只有第一次確認的 `base_hp` 下降，才能說明當時
已沒有可吸收該次傷害的虛擬護盾；仍無法反推出先前護盾剩餘量。

來源：超級對抗賽 V2.0.1，5.3 科技核心四級效果（PDF 第 85 頁）與 5.5.1
基地及前哨站機制（PDF 第 97、100 頁）。

### 基地護甲與基地增益點不可混用

「基地打開」在本倉庫的規則語境中，指**基地護甲展開**。2026 超級對抗賽
V2.0.0 已刪除「基地血量降至 `2000`（含）以下時，基地護甲展開」的舊機制；
V2.0.1 的現行展開條件有兩類：

| 類型 | 觸發條件 |
|---|---|
| 堡壘增益點 | 比賽已進行 `3` 分鐘、**己方前哨站已首次被摧毀**後，敵方一台 Hero/Infantry/Sentry 在己方堡壘增益點的單次占領時間累計 `20` 秒。機器人戰亡或暫時失去占領時，計時可保留 `3` 秒且暫停。 |
| 飛鏢 | 飛鏢選擇固定目標或隨機固定目標時，`4` 發均命中基地；選擇隨機移動目標或末端移動目標時，任意 `1` 發命中基地。命中後立即展開。 |

來源：超級對抗賽 V2.0.1，5.5.3.9（PDF 第 110-111 頁）與飛鏢機制（PDF 第 117 頁）。

裁判資料中有多組名稱相近、但語義無關的資料，策略不可互相替代：

| 資料 | 上行來源 | 真正含義 | 能否單獨判斷基地護甲展開 |
|---|---|---|---|
| `/ly/friend/base_hp`、`/ly/enemy/base_hp` | 裁判 `0x0003` 經下位機 TypeID 6/10 | 基地當前血量 | 不可以。`<= 2000` 是已刪除的舊規則。 |
| `/ly/game/event_data.self_fortress_gain_point_status` | 裁判 `0x0101` bit 25-26 | 己方堡壘增益點占領狀態 | 不可以。缺少單車 20 秒計時、前哨首毀與飛鏢命中資訊。 |
| `/ly/game/event_data.self_base_gain_point_status` | 裁判 `0x0101` bit 29 | 己方基地增益點占領狀態 | 不可以。 |
| `/ly/game/rfid.friend_base` | 裁判 `0x0209` bit 0 | 己方基地增益卡感應 | 不可以。 |

同理，`self_fortress_gain_point_status` 的 `2/3` 是**己方堡壘增益點被對方或雙方
占領**，目前只用於 `Tactical.ProtectCastle.RFID` 的防守觸發；它不表示基地護甲
狀態。

目前車載串口鏈路沒有「基地護甲已展開」的直接 ROS 欄位。BT 保存的
`selfBaseHealth` / `enemyBaseHealth` 只能讀血量；ProtectCastle 可把新鮮 `2/3`
當作前置防守告警，但不能把它標記為基地已展開。

2026 高校系列賽通信協議 V2.0.0 的另一套全局資料模型有：

- `GlobalSpecialMechanism mechanism_id=1`：己方堡壘被對方占領的計時；
- `Event event_id=13`：對方基地護甲展開。

它們目前沒有映射到下位機 TypeID 或本倉庫 ROS topic，因此不是現車決策輸入。若日後
需要精確判斷己方基地護甲展開，應由下位機/裁判資料源透傳本方等價事件，或透傳「單車
占領累計時長」及飛鏢命中結果後再由 BT 組合判定；不得用血量、RFID 或 `event_data`
代替。

### 復活後無敵差異

2026 超級對抗賽 V1.4.2 的復活後無敵規則：

| 復活方式 | 無敵 | 血量 | 其他限制 |
|---|---:|---:|---|
| 完成復活讀條 | 最長 30 秒；檢測到可占領的前哨/基地/補給區增益卡後解除虛弱，若無敵已累計超過 10 秒則解除，否則滿 10 秒後解除 | 上限血量 10% | 發射機構鎖定，不能占領增益點，不能重建前哨 |
| 金幣立即復活 | 3 秒 | 上限血量 100% | 發射機構立即解鎖，底盤功率上限短暫提高 |

2025 超級對抗賽 V2.1.0 的復活後無敵規則：

| 復活方式 | 無敵 | 血量 | 其他限制 |
|---|---:|---:|---|
| 完成復活讀條 | 哨兵 60 秒，其餘機器人 10 秒；哨兵占領補給區增益點時解除 | 上限血量 10% | 發射機構鎖定，占領補給區後解鎖 |
| 金幣立即復活 | 3 秒 | 上限血量 100% | 發射機構立即解鎖，底盤功率上限短暫提高 |

對 BT 選敵的影響：

- 2026 不能再把讀條復活後的無敵簡化成固定 10 秒；更保守的默認應按 30 秒處理。
- 2025 對哨兵有特殊 60 秒無敵；如果回放或適配 2025 規則，需要單獨把 `SentryRespawnInvulnerableSec` 配到 60。
- 立即復活通常會回滿血；現有 BT 只在敵方血量從 0 恢復到非滿血時推斷讀條復活無敵，所以不會把回滿血的立即復活按 30/60 秒處理。

來源：2026 超級對抗賽 V1.4.2，PDF page 77；2025 超級對抗賽 V2.1.0，PDF page 84。
