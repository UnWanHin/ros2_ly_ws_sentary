...
| Prefix | 边界 | 说明 |
|---|---|---|
| `/ly/control/*` | Embedded-facing | 上位机控制输入，`gimbal_driver` 按 topic 写入 `DownlinkTypeID=0x00~0x05` 对应 frame。 |
...
| Topic | Type | Consumer | 结构/语义 |
|---|---|---|---|
| `/ly/log/gimbal_raw_rx` | `gimbal_driver/msg/GimbalRawFrame` | 调试/rosbag | 下位机 -> 上位机 raw `TypedMessage`，`type_id=0..11`，`data` 是原始 bytes。 |
| `/ly/log/gimbal_raw_tx` | `gimbal_driver/msg/GimbalRawFrame` | 调试/rosbag | 上位机 -> 下位机 raw downlink frame；`type_id=250` 表示 MPC trajectory 诊断，`255..251` 表示既有 frame 诊断，`data` 是真实串口 bytes。 |
...
| `/ly/navi/should_rotate` | `std_msgs/msg/Bool` | external navigation -> `behavior_tree`；debug_node bridge（`navi_mode=true`） | 区域兼容旋转控制；true=恢复 BT 正常小陀螺/巡逻，false=关闭小陀螺并请求 `FollowMode`。`Navi.SetPostureToMoveWhenFalse=true` 时，新鲜 false 也只在 BT 本拍请求 Move 姿态，仍经既有姿态冷却/pending 机制。debug bridge 以 100 Hz 合并到完整 `/ly/control/firecode` snapshot。 |
| `/ly/navi/speed_level` | `std_msgs/msg/UInt8` | `behavior_tree` -> external navigation | 仅由 `Navi.Is_pub_navi_speed_level=true` 启用；0=停、1=正常、2=高速，非法值发布为 1。与 `/ly/control/vel` 和 `gimbal_driver` 串口下发无关。 |
...
- 正式 BT 链中，`/ly/navi/should_rotate` 的新鲜度由 `Navi.FreshTimeoutMs` 控制；新鲜 `true` 会按配置清掉外部置入的 `FollowMode`，但当前默认保留 regional 区域任务 FaceMode。`SetPostureToMoveWhenFalse=true` 时，只有仍新鲜的 false 会覆盖本 tick 的期望姿态；若姿态冷却尚未允许切换而消息已变 true 或过期，就不会补发 Move。默认超时后按允许旋转处理，但不继续保留旧 `false`。driver-only `navigation_mode` 是不做 freshness arbitration 的调试 bypass，仅在启用时采用最后一笔 `should_rotate`。
