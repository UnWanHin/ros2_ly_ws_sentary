...
**Behavion 更新**: 正式主鏈路已切成 decision-only。相機、檢測、追蹤、彈道和 fire 判斷由外部 `sentry.aim` 提供；本倉 `sentry_all` 只啟動 `gimbal_driver`、`tf_tree`、導航/FaceMode bridge 和 `behavior_tree`，不再啟動內部 `detector/tracker_solver/predictor/outpost_hitter/buff_hitter`。
