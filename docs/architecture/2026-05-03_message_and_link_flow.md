...
- `/ly/navi/target_official` - 导航 TF bridge 将有效 `/ly/navi/target_rel` 和 `/ly/aim/armor_targets` array 中每个有效 target point 反算成 official-map cm，`StampedUInt16MultiArray data=[official_x_cm, official_y_cm, armor_type]`；BT 只在对应敌方没有新鲜非零 `/ly/position/data` 时用作 `/ly/enemy/info` 和 `enemyRobots` 的位置 fallback
...
`FollowMode=1` 时，`behavior_tree` 会停小陀螺、停巡逻扫描、停止新的开火翻转，并保持当前云台角。FaceMode 区域接管只停云台巡逻/开火并使用固定点角度，不会单独停小陀螺。启用 `Navi.yaml` 的 `Navi.Enable` 后，外部导航可用 `/ly/navi/should_rotate=false` 临时请求 `FollowMode+Rotate=0`，再用新鲜 `true` 恢复 BT 对旋转档位的正常决策；当前默认保留 regional 区域任务 FaceMode，不影响 Buff/Outpost AimMode 自己的固定朝向。`Navi.Is_pub_navi_speed_level=true` 时，BT 另向外部导航发布 `/ly/navi/speed_level`，不参与 gimbal 串口下发。
...
3. **導航状态**:
   ...
   - `/ly/navi/should_rotate` - 外部导航区域兼容旋转控制
