...
namespace ly_auto_aim::inline detector {
    ...
    bool ArmorRefinder::ReFindAndSolveAll(PoseSolver& solver, const std::vector<ArmorObject>& armors, const ArmorType& target,ArmorObject& target_armor,  auto_aim_common::Armors& armors_msg) noexcept{
        if (armors.empty()) return false;

        min_offset = FLT_MAX;
        min_index = 100; // 表示一个不存在的下标
        constexpr auto thread_hold = 100;
        auto_aim_common::Armor temp_armor_msg;
