...
void Tracker::merge(const Detections& detections, double threshold) 
{
    ...
    for(const auto& detection : detections) {
        ...
        // if(detection.isGray)  /// 这个地方需要修改，因为已经不检测Gray了
...
TrackResults Tracker::getArmorTrackResult(const Time::TimeStamp& time, const GimbalAngleType& gimbalAngle) 
{
    ...
    std::map<int,std::vector<std::pair<cv::Point2f,const ArmorXYV*>>> params;//(car_id, centers)
    for(const auto& armor : armors) {
        ...
        //center_location.gimbalAngle = gimbalAngle;
        //CXYD coord = center_location.cxy_gimbalAngle;
        CXYD coord = center_location.getImuCXY(gimbalAngle);   /// 后面记得手动更新
    ...
    for(auto& [car_id, param] : params) {
        ...
        for(auto& [id, armor_ptr] : result) {
            ...
            //roslog::info("Car {} Armor {} at ({}, {})", car_id, id, track_result.extra_center.x, track_result.extra_center.y);
...
CarTrackResults Tracker::getCarTrackResult(const Time::TimeStamp& time, const GimbalAngleType& gimbalAngle, const TrackResults& armor) 
{
    // 第一阶段：旧车辆矩形匹配
...
TrackResults Tracker::initArmorTrackResult(const GimbalAngleType& gimbalAngle) 
{
...
void Tracker::getArmorTrackResultWithWholeCar(const Time::TimeStamp& time, const GimbalAngleType& gimbalAngle, const CarTrackResults& car_results, TrackResults& armor_results)
{
