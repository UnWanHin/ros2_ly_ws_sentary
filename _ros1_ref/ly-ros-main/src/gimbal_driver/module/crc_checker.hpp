...
public:
    ...
    class CRC8 {
    private:
        static constexpr unsigned char CRC8_INIT = 0xFF;
    ...
    public:
        ...
        static void append(uint8_t* data, uint32_t length) {
            if(length < 1) return;
            data[length-1] = calculate(data, length-1);
        }

        // 验证数据完整性
        static bool verify(const uint8_t* data, uint32_t length) {
            if(length < 1) return false;
            return calculate(data, length-1) == data[length-1];
        }
    };

    // CRC16校验功能组（使用CRC-16/IBM算法）
    class CRC16 {
    private:
    ...
    public:
        ...
        static uint16_t calculate(const uint8_t* data, uint32_t length) {
            ...
            while(length--) {
                wCRC = (wCRC >> 8) ^ wCRC_Table[(wCRC ^ *data++) & 0x00FF];
            }
            return wCRC;
        }
        ...
        static void append(uint8_t* data, uint32_t length) {
            if(length < 2) return;
            uint16_t crc = calculate(data, length-2);
        ...
        static bool verify(const uint8_t* data, uint32_t length) {
            ...
            return calculate(data, length-2) == 
                  (static_cast<uint16_t>(data[length-1]) << 8 | data[length-2]);
        }
    };
};