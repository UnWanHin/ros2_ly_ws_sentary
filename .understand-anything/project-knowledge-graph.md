...
- `io_config.navigation_mode` 的預設單節點 profile 是 `src/gimbal_driver/config/debug_mode.yaml`，由 `debug_node.launch.py` 載入；`gimbal_driver` 直接把 `/ly/navi/vel` 與 `/ly/navi/should_rotate` 轉為下位機控制，正式導航控制仍經 BT 發布 `/ly/control/*`。此入口不啟動 BT，故不消費也不宣告 `SetPostureToMoveWhenFalse`。正式 root 相容路由明確略過 `navigation_test`／`navigation_mode`。
