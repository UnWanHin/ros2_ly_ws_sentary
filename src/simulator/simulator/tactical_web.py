...
function renderTactical() {{ const tactical=state.tactical || {{}}; const castle=tactical.protect_castle || {{}}; const hero=tactical.protect_hero || {{}}; const defense=tactical.regional_defense || {{}}; addKv(document.getElementById('tactical'), [['Evidence',tactical.available ? 'BT trace' : 'not recorded'],['Castle',castle.enabled],['RFID source',castle.rfid_event_active],['RFID stay',castle.stay_when_rfid_active],['Enemy in base',castle.enemy_pos_active],['Hero protection',hero.active],['Defense threat',defense.threat_active],['Defense search',defense.search_kind],['Base enemies',defense.own_base_enemy_count]]); }}
