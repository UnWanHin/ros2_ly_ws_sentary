...
"""
纯打前哨联调入口（不启动 behavior_tree，不做巡逻/策略导航）。
...
def generate_launch_description():
    ...
    bridge_use_target_status = LaunchConfiguration("bridge_use_target_status")
    ...
    bridge_bt_target = LaunchConfiguration("bridge_bt_target")
    ...
    launch_args = [
        ...
        DeclareLaunchArgument("bridge_target_topic", default_value="/ly/outpost/target"),
        DeclareLaunchArgument("bridge_enable_fire", default_value="true"),
        DeclareLaunchArgument("bridge_use_target_status", default_value="false"),
        ...
        DeclareLaunchArgument("bridge_bt_target", default_value="7"),
    ]

    info_logs = [
        LogInfo(msg=["[outpost_test] config: ", config_file]),
        LogInfo(msg=["[outpost_test] base_config: ", base_config_file]),
        LogInfo(msg=["[outpost_test] detector_config: ", detector_config_file]),
        LogInfo(msg=["[outpost_test] outpost_config: ", outpost_config_file]),
        LogInfo(msg=["[outpost_test] output: ", output]),
        LogInfo(msg=["[outpost_test] bridge_target_topic: ", bridge_target_topic]),
        LogInfo(msg=["[outpost_test] bridge_enable_fire: ", bridge_enable_fire]),
        LogInfo(msg=["[outpost_test] bridge_use_target_status: ", bridge_use_target_status]),
        LogInfo(msg=["[outpost_test] bridge_fire_hz: ", bridge_fire_hz]),
    ]
    ...
    nodes = [
        ...
        Node(
            ...
            parameters=[{
                ...
                "use_target_status": ParameterValue(bridge_use_target_status, value_type=bool),
            ...
            on_exit=Shutdown(reason="outpost_test_bridge exited"),
