...
public:
  ...
  : Node("tf_tree_node")
  {
    ...
    big_yaw_offset_deg_ = get_parameter("big_yaw_offset_deg").as_double();
    const auto gimbal_topic = get_parameter("gimbal_topic").as_string();
    const auto gimbal_big_yaw_topic = get_parameter("gimbal_big_yaw_topic").as_string();
    ...
    RCLCPP_INFO(
      get_logger(),
      "tf_tree_node started. topic=%s, chain: %s -> %s and %s --static(+Z)-> %s --pitch-> %s "
...
private:
  ...
  void BroadcastBigYawTf(const std_msgs::msg::Float32 & msg)
  {
    const double yaw_rad =
      -(static_cast<double>(msg.data) + big_yaw_offset_deg_) * M_PI / 180.0;
    ...
    ts.header.stamp = t;
    ts.header.frame_id = big_yaw_frame_;
    ts.child_frame_id = yaw_frame_;
    ts.transform.translation.x = 0.0;
    ts.transform.translation.y = 0.0;
    ts.transform.translation.z = 0.0;
    ts.transform.rotation.x = q_yaw_inverse.x();
    ts.transform.rotation.y = q_yaw_inverse.y();
    ts.transform.rotation.z = q_yaw_inverse.z();
    ts.transform.rotation.w = q_yaw_inverse.w();
  ...
  rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr gimbal_big_yaw_sub_;
  std::string world_frame_;
  std::string yaw_frame_;
  std::string big_yaw_frame_;
  std::string barrel_joint_frame_;
  std::string barrel_frame_;
  double barrel_offset_z_{0.0};
  double big_yaw_offset_deg_{0.0};
};
